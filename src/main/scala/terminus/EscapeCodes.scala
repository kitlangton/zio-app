...
class EscapeCodes(out: OutputStream) {
  ...
  private def ESC(command: Char) { out.write(("\u001b" + command).getBytes) }
  // Output a Control Sequence Inroducer
  private def CSI(sequence: String) {
  ...
  private def CSI(n: Int, m: Int, o: Int, command: Char) {
    CSI(s"$n;$m;$o$command")
  }
  // Execute commands in private modes
  private def CSI(mode: Char, command: Char) { CSI(s"$mode$command") }
  private def CSI(mode: Char, n: Int, command: Char) {
