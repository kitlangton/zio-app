  ...
  def client_impl[Service: c.WeakTypeTag]: c.Tree = {
    ...
    val methodDefs = serviceType.decls.collect { case method: MethodSymbol =>
      ...
      val request =
            if (isStream) {
              if (params.isEmpty)
                q"""_root_.zio.app.FrontendUtils.fetchStream[$errorType, $returnType](config.getString("zio.app.serverhost"), ${serviceType.finalResultType.toString}, ${methodName.toString})"""
              else
                q"""_root_.zio.app.FrontendUtils.fetchStream[$errorType, $returnType](config.getString("zio.app.serverhost"), ${serviceType.finalResultType.toString}, ${methodName.toString}, Pickle.intoBytes($pickleType))"""
            } else {
              if (params.isEmpty)
                q"""_root_.zio.app.FrontendUtils.fetch[$errorType, $returnType](config.getString("zio.app.serverhost"), ${serviceType.finalResultType.toString}, ${methodName.toString})"""
              else
                q"""_root_.zio.app.FrontendUtils.fetch[$errorType, $returnType](config.getString("zio.app.serverhost"), ${serviceType.finalResultType.toString}, ${methodName.toString}, Pickle.intoBytes($pickleType))"""
            }
